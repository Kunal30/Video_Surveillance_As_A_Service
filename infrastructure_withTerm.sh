...
if [ "$1" == "create" ]; then
	...
	scp    -i ~/.ssh/$vs_key_pair_name.pem -o StrictHostKeyChecking=no    ./AppTier_Terminator/pom.xml ubuntu@$app_instance_ip:~/
	scp -r -i ~/.ssh/$vs_key_pair_name.pem -o StrictHostKeyChecking=no    ./AppTier_Terminator/src ubuntu@$app_instance_ip:~/
	...
	ssh    -i ~/.ssh/$vs_key_pair_name.pem -o StrictHostKeyChecking=no -t ubuntu@$app_instance_ip mv ./target/AppTier_Terminator-1.0.0.jar ./darknet > /dev/null
	ssh    -i ~/.ssh/$vs_key_pair_name.pem -o StrictHostKeyChecking=no -t ubuntu@$app_instance_ip java -jar ./darknet/AppTier_Terminator-1.0.0.jar &
